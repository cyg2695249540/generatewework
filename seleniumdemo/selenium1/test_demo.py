...
class TestDemo:
    ...
    @pytest.mark.parametrize("username,acctid,phone,result",[("aaa",12345,13711111111,"保存成功")],ids={"添加成员成功"})
    def test_addmember(self,username,acctid,phone,result):
        ...
        sleep(1)
        getresult = self.driver.find_element(By.CSS_SELECTOR, ".success").text
    ...
    @pytest.mark.parametrize("username,acctid,phone", [("bbb", 123456, 13711111112)], ids={"添加成员失败"})
    ...
    @pytest.mark.parametrize("username,acctid,phone,result", [("ccc", 1234567, 13711111113, "保存成功")], ids={"通讯录添加成员成功"})
    ...
    @pytest.mark.parametrize("username,result", [("aaa", "删除成功"),("ccc", "删除成功")], ids={("删除成员aaa成功"),("删除成员ccc成功")})
