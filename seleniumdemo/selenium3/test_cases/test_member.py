...
class TestMember:
    ...
    def test_addmember(self, username, acctid, phone, result):
        result_tips = self.main.goto_addmember_page().addusername(username).addacctid(acctid).addphone(phone).save_member().get_tips_text()
    ...
    def test_delete_member(self, username1,username2, result):
        get_tips = self.main.goto_contact_page().deletemember(username1,username2).get_tips_text()
        assert get_tips == result
        namelist = self.main.goto_contact_page().get_member_list()
